...
function validateDeckType(id, deck) {
    ...
    if (!Array.isArray(deck.main) || !Array.isArray(deck.egg) || !Array.isArray(deck.side)) {
...
export async function saveDeck(jwt, deck, owner, callback) {
    const result = { error: null, decks: [] }
    try {
...
export async function deleteDeck(jwt, deckID, owner, callback) {
    let results = { error: null, decks: [] };
    try {
