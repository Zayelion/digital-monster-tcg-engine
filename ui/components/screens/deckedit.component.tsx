...
import CardInfo from '../game/cardinfo.component';
import { cardStackSort, cardEvaluate, deepShuffle, isEgg } from '../../util/cardManipulation';
import { hey, listen, watchOut } from '../../services/listener.service';
import { CardImage } from '../common/card.component';
import { getCardDatabase, getSetcodes, getSets } from '../../services/carddatabase.service';
...
function CardList(activeDeck) {
  const Main = listReduce(activeDeck.main),
    ...
    Egg = listReduce(activeDeck.egg);
  return (
    <div id="decktextlist" key="deckedit-decktextlist-div">
      <h4>{`Main Deck - ${activeDeck.main.length}x`}</h4>
      <Main />
      <h4>{`Side Deck - ${activeDeck.side.length}x`}</h4>
      <Side />
      <h4>{`Egg Deck - ${activeDeck.egg.length}x`}</h4>
      <Egg />
    </div>
  );
}
...
function checkLegality(card, zone, deck, banlist) {
  ...
  var masterRule = banlist.masterRule,
    ...
    eggCount = deck.egg.filter(checkCard).length,
    sideCount = deck.side.filter(checkCard).length;

  if (mainCount + eggCount + sideCount >= card.limit) {
    return false;
  }
  ...
  if (zone === 'egg' && deck[zone].length >= 15) {
    return false;
  }
  return true;
}
...
function VisualizedDeck(initialDeck) {
  const [name, setName] = useState(initialDeck?.name),
    main = VisualizedList(initialDeck?.main),
    egg = VisualizedList(initialDeck?.egg),
    side = VisualizedList(initialDeck?.side);
...
export default function DeckEditScreen() {
  ...
  function applyBanlist() {
    ...
    result = Object.keys(map).map(function (id) {
      map[id].limit =
        activeBanlist.bannedCards[id] !== undefined ? parseInt(activeBanlist.bannedCards[id], 10) : 3;
      return map[id];
    });
  ...
  useEffect(() => {
    ...
    listen('LOAD_DECKS', (action) => {
      ...
      setSearch([]);
      setDecks(
        action.decks.map((deckIds) => {
          const deck = Object.assign({}, deckIds);
          deck.main = deck.main.map(findcard);
          deck.egg = deck.egg.map(findcard);
          deck.side = deck.side.map(findcard);

          return deck;
        })
      );
      setActiveDeck(VisualizedDeck(decks[settings.decklist]));
    });

    watchOut('LOAD_DATABASE', (action) => {
      debugger;
      loadDatabase(getCardDatabase());
      fullDatabse = getCardDatabase();
      info.databaseSystem = getCardDatabase();
      setSetcodes(getSetcodes());
      setReleases(getSets());
      searchDB();
    });
  ...
  function sort() {
    ...
    activeDeck.egg.sort(cardEvaluate);
  ...
  function exportDeck() {
    ...
    file += '#egg\r\n';
    activeDeck.egg.forEach(printCard);
    file += '!side\r\n';
  ...
  function onCardDoubleClick(source, index, event) {
    ...
    if (source === 'search') {
      ...
      if (isEgg(card)) {
        legal = checkLegality(card, 'egg', activeDeck, banlist);
        activeDeck.egg.add(card);
  ...
  function CardTypes() {
    switch (settings.cardtype) {
        ...
        return (
          <>
            <select id="type1" onChange={onSearchChange}>
              <option value={undefined}>Frame</option>,<option value={0x40}>Fusion</option>
            ...
            <select id="type2" onChange={onSearchChange}>
              ...
              <option value={0x400000}>Toon</option>
            </select>
      ...
      case 2: // Spells
        return (
          <select id="exacttype" onChange={onSearchChange}>
            ...
            <option value={524290}>Equip</option>
          </select>
        );
      case 4: //Traps
        return (
          <select id="exacttype" onChange={onSearchChange}>
            ...
            <option value={1048580}>Counter</option>
          </select>
        );
      default:
        return <></>;
  ...
  function onDropDeckZone(zone, event) {
    ...
    if (zone === 'egg' && !isEgg(card)) {
      return;
    }
    if (zone === 'main' && isEgg(card)) {
      return;
    }
    if (source === 'search') {
  ...
  function Releases() {
    ...
    return (
      <>
        {[
          <option value={undefined} key={'set-u}'}>
            Release Set
          </option>
        ].concat(list)}
      </>
  ...
  function SearchControls() {
    return (
      <div key="deckedit-card-controls">
        <div key="deckedit-col-1" className="filtercol">
          <select key="deckedit-cardtype" id="cardtype" onChange={onSearchChange}>
            <option key="deckedit-cardtype-1" value={5}>
              Monster/Spell/Trap
            </option>
            <option key="deckedit-cardtype-2" value={1}>
              Monster
            </option>
            <option key="deckedit-cardtype-3" value={2}>
              Spell
            </option>
            <option key="deckedit-cardtype-4" value={4}>
              {' '}
              Trap
            </option>
          </select>
          <div className="filtercol" key="deckedit-filtercol-2">
            <CardTypes />
          </div>

          <select key="deckedit-setcode" id="setcode" onChange={onSearchChange}>
            {[
              <option key={'u'} value={undefined}>
                Archetype
              </option>
            ].concat(
              setcodes.map((list, i) => {
                return (
                  <option key={`setcode-${i}`} value={parseInt(list.num)}>
                    list.name
                  </option>
                );
              })
          ...
          <select key="deckedit-limit" id="limit" onChange={onSearchChange}>
            <option value="null" key="deckedit-limit-null">
              Limit
            </option>
            <option value={3} key="deckedit-limit-0">
              Unlimited
            </option>
            <option value={2} key="deckedit-limit-1">
              Semi-Limited
            </option>
            <option value={1} key="deckedit-limit-2">
              Limited
            </option>
            <option value={0} key="deckedit-limit-3">
              Forbidden
            </option>
          </select>
          <input
          ...
          />
          <Stats />
          <button key="deckedit-clearsearch" onClick={clearSearch}>
            Reset
          </button>
        </div>
        <LinkArrows />
      ...
      <controls key="deck-banlist-controls">
        ...
        <div className="filtercol">
          ...
          <select id="banlist" key="deckedit-ban;list-select" onChange={onSearchChange}>
            {banlist.map((list, i) => {
              return (
                <option value={list.name} key={i} selected={list.primary}>
                  {list.name}
                </option>
              );
            })}
        ...
        <div className="deckcontrols" key="deckedit-deckcontrols-div1">
          ...
          <div className="deckcontrols" key="deckedit-deckcontrols-div2">
            <button key="deckedit-decklist-0" onClick={newDeck}>
              New
            </button>
            <button key="deckedit-decklist-1" onClick={save}>
              Save
            </button>
            <button key="deckedit-decklist-2" onClick={deleteDeck}>
              Delete
            </button>
            <button key="deckedit-decklist-3" onClick={rename}>
              Rename
            </button>
            <button key="deckedit-decklist-4" onClick={clear}>
              Clear
            </button>

            <button key="deckedit-decklist-5" onClick={sort}>
              Sort
            </button>
            <button key="deckedit-decklist-6" onClick={shuffle}>
              Shuffle
            </button>
            <button key="deckedit-decklist-7" onClick={exportDeck}>
              Export
            </button>
            <button key="deckedit-decklist-8" onClick={saveAs}>
              Save As
            </button>
          </div>
        </div>
      ...
      <div id="deckarea">
        ...
        <div id="deckareaegg">
          ...
          <div
            ...
            onDragOver={onDragOver}
            onDrop={onDropDeckZone.bind(this, 'egg')}
          >
            <CardCollection source="egg" deck={activeDeck.egg} />
          </div>
        </div>
        <div id="deckareaside">
          <h2>Side Deck</h2>
